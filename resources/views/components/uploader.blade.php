...
<div class="bg-white h-auto w-auto sm:px-8 md:px-16 sm:py-8">
  ...
  <template id="file-template">
    <li class="block p-1 w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/6 xl:w-1/8 h-72">
...
const fileTempl = document.getElementById("file-template"),
  ...
  attachment_container = document.getElementById('attachment-container');
...
attachment_container.ondrop = (event) => {
...
attachment_container.ondragover = (event) => {
  event.preventDefault();
  if (!hasFiles(event)) {
    return;
  }
...
attachment_container.ondragleave = (event) => {
  1 > --counter && overlay.classList.remove("draggedover");
}

attachment_container.ondragenter = (event) => {
  if (hasFiles(event)) {
    e.preventDefault();
  }
}
